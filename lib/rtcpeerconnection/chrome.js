...
function ChromeRTCPeerConnection(configuration, constraints) {
  ...
  util.interceptEvent(this, 'icecandidate');
  ...
  peerConnection.addEventListener("icecandidate", function onicecandidate() {
    var args = [].slice.call(arguments);
    var candidate = args[0].candidate;
    if (candidate != null)
      console.log("#Swi.js.tw: onIceCandidate", candidate.candidate, candidate.sdpMLineIndex, candidate.sdpMid);
    else
      console.log("#Swi.js.tw: onIceCandidate", null);
    self.dispatchEvent.apply(self, arguments);
  });
  ...
  console.log("#Swi.js.tw: addTrack=", typeof peerConnection.addTrack);
}

inherits(ChromeRTCPeerConnection, EventTarget);

if (typeof PeerConnection.prototype.addTrack !== 'function') {
  console.log("#Swi.js.tw: addTrack reimplemented");
...
ChromeRTCPeerConnection.prototype.addIceCandidate = function addIceCandidate(candidate) {
  ...
  if (this.signalingState === 'have-remote-offer') {
    ...
    console.log("#Swi.js.tw: addIceCandidate_high: ", candidate.candidate, candidate.sdpMLineIndex, candidate.sdpMid);
    promise = this._signalingStateLatch.when('low').then(function signalingStatesResolved() {
      console.log("#Swi.js.tw: addIceCandidate_low: ", candidate.candidate, candidate.sdpMLineIndex, candidate.sdpMid);
      return self._peerConnection.addIceCandidate(candidate);
    });
  } else {
    console.log("#Swi.js.tw: addIceCandidate: ", candidate.candidate, candidate.sdpMLineIndex, candidate.sdpMid);
...
ChromeRTCPeerConnection.prototype.createAnswer = function createAnswer() {
  ...
  if (this._pendingRemoteOffer) {
    ...
    var gmoOffer = this._pendingRemoteOffer;
    ...
    promise = this._peerConnection.setRemoteDescription(this._pendingRemoteOffer).then(function setRemoteDescriptionSucceeded() {
      ...
      console.log("#Swi.js.tw: setRemoteDescription_low + createAnswer: ", gmoOffer.type, gmoOffer.sdp, answer.type, answer.sdp);
      self._pendingRemoteOffer = null;
      var a = new RTCSessionDescription({
        type: 'answer',
        sdp: updateTrackIdsToSSRCs(self._sdpSemantics, self._tracksToSSRCs, answer.sdp)
      });
      console.log("#Swi.js.tw: hooked answer: ", a.type, a.sdp);
      return a;
    ...
    promise = this._peerConnection.createAnswer().then(function(answer) {
      console.log("#Swi.js.tw: createAnswer: ", answer.sdp);
...
ChromeRTCPeerConnection.prototype.createOffer = function createOffer() {
  ...
  var promise = this._peerConnection.createOffer(options).then(function(offer) {
    console.log("#Swi.js.tw: createOffer: ", offer.type, offer.sdp);
    let a = new ChromeRTCSessionDescription({
      type: offer.type,
      sdp: updateTrackIdsToSSRCs(self._sdpSemantics, self._tracksToSSRCs, offer.sdp)
    });
    console.log("#Swi.js.tw: createOffer (hooked): ", a.type, a.sdp);
    return a;
  });
...
ChromeRTCPeerConnection.prototype.setLocalDescription = function setLocalDescription() {
  ...
  console.log("#Swi.js.tw: setLocalDescription_high: ", description.type, description.sdp);
...
ChromeRTCPeerConnection.prototype.setRemoteDescription = function setRemoteDescription() {
  var args = [].slice.call(arguments);
  var description = args[0];
  console.log("#Swi.js.tw: setRemoteDescription_high: ", description.type, description.sdp);
...
function setRemoteAnswer(peerConnection, answer) {
  ...
  console.log("#Swi.js.tw: setLocalDescription_low: ", pendingLocalOffer.type, pendingLocalOffer.sdp);
  return peerConnection._peerConnection.setLocalDescription(pendingLocalOffer).then(function setLocalOfferSucceeded() {
    peerConnection._pendingLocalOffer = null;
    console.log("#Swi.js.tw: setRemoteDescription_low: ", answer.type, answer.sdp);
